	...
	class Danscii {
		...
		_buildDom() {
			...
			this._onPointerMove = (e) => {
				// Cache rect — getBoundingClientRect every mousemove forces layout in Safari.
				let rect = this._pointerRect;
				if (!rect) {
					rect = this.canvas.getBoundingClientRect();
					this._pointerRect = rect;
				}
		...
		_updateActivation() {
			...
			const hoverEnabled = hoverCells > 0;

			const applyCell = (idx, near) => {
				const prevA = activation[idx];
				const prevT = trail[idx];
				const prevD = display[idx];
				if (near) {
					activation[idx] = Math.min(1, activation[idx] + 0.35);
					if (trail[idx] < 1) {
						trail[idx] = 1;
						this._stampTrailCell(idx);
					} else {
						trail[idx] = 1;
					}
				} else {
					...
					if (!persist && trail[idx] > 0) {
						...
						if (prevT > 0 && trail[idx] === 0) this._baseDirty = true;
					} else if (!hoverEnabled && persist && trail[idx] > 0) {
						trail[idx] = 0;
						this._baseDirty = true;
					}
				}
				...
				if (
					activation[idx] !== prevA ||
					trail[idx] !== prevT ||
					display[idx] !== prevD ||
					(prevA > 0.04) !== (activation[idx] > 0.04)
				) {
					changed = true;
				}
			};
			...
			if (!hovering || !hoverEnabled) {
				for (let idx = 0; idx < activation.length; idx++) {
					...
					if (activation[idx] === 0 && (persist || trail[idx] === 0) && hoverEnabled) continue;
					if (activation[idx] === 0 && trail[idx] === 0 && !hoverEnabled) continue;
					applyCell(idx, false);
				}
				return changed;
			}
			...
			const reachPad = Math.ceil(baseReach * 1.6) + 1;
			const rowMin = Math.max(0, Math.floor(hoverRow) - reachPad);
			const rowMax = Math.min(gridRows - 1, Math.ceil(hoverRow) + reachPad);
			const colMin = Math.max(0, Math.floor(hoverCol) - reachPad);
			const colMax = Math.min(gridCols - 1, Math.ceil(hoverCol) + reachPad);
			...
			for (let row = rowMin; row <= rowMax; row++) {
				for (let col = colMin; col <= colMax; col++) {
					...
					applyCell(idx, dist <= jaggedReach);
				}
			}

			// Decay cells that were activated but are now outside the brush.
			for (let idx = 0; idx < activation.length; idx++) {
				if (activation[idx] === 0 && (persist || trail[idx] === 0)) continue;
				if (target[idx] === " ") continue;
		...
		_stampTrailCell(idx) {
			if (!this._introDone || this._baseDirty || !this.target) return;
			this._syncBaseLayer();
			const bctx = this._bctx;
			if (!bctx) return;
			const ch = this.target[idx];
			if (!ch || ch === " ") return;
			const dpr = window.devicePixelRatio || 1;
			const col = idx % this.gridCols;
			const row = (idx / this.gridCols) | 0;
			bctx.setTransform(dpr, 0, 0, dpr, 0, 0);
			bctx.font = `${this.fontSize}px ${this.options.fontFamily || "monospace"}`;
			bctx.textBaseline = "middle";
			bctx.textAlign = "center";
			bctx.fillStyle = this.getColors().hover;
			bctx.fillText(
				ch,
				col * this.cellW + this.cellW * 0.5,
				row * this.cellH + this.cellH * 0.5,
			);
		}
		...
		_bakeBase() {
			this._syncBaseLayer();
			const bctx = this._bctx;
			if (!bctx || !this.target) return;
			const dpr = window.devicePixelRatio || 1;
			const { cellW, cellH, gridCols, gridRows, target, trail } = this;
			const c = this.getColors();
			bctx.setTransform(1, 0, 0, 1, 0, 0);
			bctx.clearRect(0, 0, this._base.width, this._base.height);
			bctx.setTransform(dpr, 0, 0, dpr, 0, 0);
			bctx.font = `${this.fontSize}px ${this.options.fontFamily || "monospace"}`;
			bctx.textBaseline = "middle";
			bctx.textAlign = "center";

			// Settled glyphs only (target). Live scramble is drawn in the overlay pass.
			bctx.fillStyle = c.fg;
			for (let row = 0; row < gridRows; row++) {
				const y = row * cellH + cellH * 0.5;
				const off = row * gridCols;
				for (let col = 0; col < gridCols; col++) {
					const idx = off + col;
					const ch = target[idx];
					if (!ch || ch === " ") continue;
					if (trail && trail[idx] > 0) continue;
					bctx.fillText(ch, col * cellW + cellW * 0.5, y);
				}
			}
			bctx.fillStyle = c.hover;
			for (let row = 0; row < gridRows; row++) {
				const y = row * cellH + cellH * 0.5;
				const off = row * gridCols;
				for (let col = 0; col < gridCols; col++) {
		...
		_drawFull() {
			...
			for (let row = 0; row < gridRows; row++) {
				...
				for (let col = 0; col < gridCols; col++) {
					...
					ctx.fillText(ch, col * cellW + cellW * 0.5, y);
				}
			}
		}
		...
		_draw() {
			...
			// Intro scrambles the whole field — full redraw is simpler and short-lived.
			if (!this._introDone) {
				this._drawFull();
				return;
			}
			...
			this._syncBaseLayer();
			if (this._baseDirty) this._bakeBase();
			...
			const { cellW, cellH, gridCols, gridRows, display, activation, settle, matrixHeat } = this;
			const c = this.getColors();
			ctx.font = `${this.fontSize}px ${this.options.fontFamily || "monospace"}`;
			ctx.textBaseline = "middle";
			ctx.textAlign = "center";
			...
			for (let row = 0; row < gridRows; row++) {
				...
				for (let col = 0; col < gridCols; col++) {
					const idx = off + col;
					const hoverLit =
						(activation && activation[idx] > 0.04) || (settle && settle[idx] > 0);
					const matrixLit = matrixHeat && matrixHeat[idx] > 0.04;
					if (!hoverLit && !matrixLit) continue;
					const ch = display[idx];
					if (!ch || ch === " ") continue;
					ctx.fillStyle = hoverLit ? c.hover : c.matrix;
					ctx.fillText(ch, col * cellW + cellW * 0.5, y);
				}
			}
		}
